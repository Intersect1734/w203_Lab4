...
First we identify if our data fits our model assumptions.  As our model uses a "carrot" and "stick" approach we investigate the former and later variables.  We identify that police presense ("stick") is appropriate for our model, but our "carrot" is potentially of concern as we are taking the "average of averages" in this context.  We are presented with no other data and consequently will proceed with the understanding that this may limit the full interpretability of our model.
...
plot(Model1, 1)
```
From our plot, we observe a spline curve that is curved on the right.  This normally indicates a violation of Zero Conditional Mean; however, we observe that there are very few data points on the right of the of the graph which could contribute to this curve.  We consequently will assume Zero Conditional Mean, but proceed further with caution.

Our next tested assumption is MLR.5, the assumption of homoskedacity.  From the above residuals versus fitted, we can observe that the band has a roughly uniform thickness, with the exception of a handful of points on the right.  We will proceed cautiously with further analysis of homoskedasticity.

```{r,echo=FALSE, out.width='.49\\linewidth', fig.width=5, fig.height=3,fig.show='hold',fig.align='center'}
plot(Model1, 3)
```

Our scale-location plot also depicts a relatively horizontal band of points, with the exception of a handful on the right.  We will continue to proceed cautiously.

```{r}
bptest(Model1)
```
From our Breusch-Pagan test, we observe that we must reject the null hypothesis of homoskedasticity as we have a p-value of less than .05, indicating that there is evidence supporting heteroskedasticity.  

KRISSY - DO YOU SEE THIS AS AN ISSUE?

We proced with the analysis of MLR.6, The normality of errors.  

We first investigate the residual plot:

```{r,echo=FALSE, out.width='.49\\linewidth', fig.width=5, fig.height=3,fig.show='hold',fig.align='center'}
hist(Model1$residuals)
```
We observe some positive skew, but it is roughly normally distributed.

```{r,echo=FALSE, out.width='.49\\linewidth', fig.width=5, fig.height=3,fig.show='hold',fig.align='center'}
plot(Model1, 2)
```
Our QQ test shows that the majority of points falls on the diagonal line with some departure along the extremes.  

```{r}
length(data$crime)
```

As our sample approaches 100 data points, we can rely on asymptotic properties of OLS including the central limit theorem.  

We now investigate our outliers:
...
Our model takes the following form:  
$$Crime = \beta_0 + \beta_1*Police + \beta_3*Ymale + \beta_4*Density + \beta_5*PublicWages + \beta_6*PrivateWages$$  

```{r}
Model2 <- lm(crime ~ police + ymale + density + wage_public + wage_private, data = data)
